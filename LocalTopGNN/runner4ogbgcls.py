...
class Trainer(TrainerBase):
    ...
    def train_batch(self, graph_batch: PathTreeWithRingsBatch) -> float:
        ...
        if graph_batch.batch_size < 8:
            ...
            return 0.
        num_rings = graph_batch.num_rings
        if num_rings < 8:
            # print("A batch skipped")
...
def set_config(args):
    config_path = 'LocalTopGNN/config/ogbgcls.json'
...
def main():
    parser = argparse.ArgumentParser(
        description='Evaluating LocalTopGNN\'s performance on TU datasets.')
